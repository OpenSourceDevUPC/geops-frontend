...
export class CartApiEndpoint extends BaseApiEndpoint<Cart, CartResource, CartResponse, CartAssembler> {
  ...
  getByUser(userId: string) {
    return this.http.get<CartResource>(`${this.endpointUrl}/user/${encodeURIComponent(userId)}`).pipe(
   ...
   */
  addItemToUser(userId: string, item: any) {
    return this.http.post<CartResource>(`${this.endpointUrl}/user/${encodeURIComponent(userId)}/items`, item).pipe(
      map(resource => this.assembler.toEntityFromResource(resource))
    );
  }
   ...
   */
  updateItemQuantityByUser(userId: string, offerId: string, body: any) {
    return this.http.put<CartResource>(`${this.endpointUrl}/user/${encodeURIComponent(userId)}/items/${encodeURIComponent(offerId)}`, body).pipe(
      map(resource => this.assembler.toEntityFromResource(resource))
    );
  }
   ...
   */
  createCartForUser(userId: string, resource?: CartResource) {
    const url = `${this.endpointUrl}?userId=${encodeURIComponent(userId)}`;
    return this.http.post<CartResource>(url, resource || {}).pipe(
